...
type Props = {
  ...
  initialPreachedAt?: string;
  initialVideoUrl?: string;
  initialAudioUrl?: string;
  initialNotesUrl?: string;
...
export default function SermonEditor({
  ...
  const [preachedAt, setPreachedAt] = useState(initialPreachedAt);
  const [videoUrl, setVideoUrl] = useState(initialVideoUrl);
  const [audioUrl, setAudioUrl] = useState(initialAudioUrl);
  const [notesUrl, setNotesUrl] = useState(initialNotesUrl);
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        slug, status, speakerName, series,
        preachedAt: preachedAt || null,
        videoUrl: videoUrl || null, audioUrl: audioUrl || null,
        notesUrl: notesUrl || null, thumbnailUrl: thumbnailUrl || null,
        translations: Object.entries(translations).map(([locale, data]) => ({ locale, ...data })),
      };
      const res = await fetch(isEdit ? `/api/sermons/${sermonId}` : '/api/sermons', {
    ...
    <div className="max-w-4xl">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={slug} onChange={(e) => { setSlug(e.target.value); setSlugTouched(true); }}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#437086]" placeholder="sermon-url-slug" />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="date" value={preachedAt} onChange={(e) => setPreachedAt(e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#437086]" />
          </div>
          <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">Notes PDF URL</label>
